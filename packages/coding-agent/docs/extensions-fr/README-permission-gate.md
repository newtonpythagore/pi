...
3. **Confirmation utilisateur** — tout le reste (donc tout ce qui peut modifier le système de fichiers ou l'état du système : `rm`/`rmdir`/`mv` simples, `find -exec`, `mkdir`, `sed -i`, redirections, `git commit`, `npm install`, one-liners python/perl, ...) déclenche un dialogue de validation en **français**, affiché en **rouge** (couleur `error` du thème), avec la commande complète. **« Non — refuser » est présélectionné** : un appui réflexe sur Entrée refuse la commande, ce qui évite les validations accidentelles. Refus, fermeture du dialogue (Échap) ou expiration → commande bloquée avec un message ferme indiquant à l'IA que l'utilisateur refuse cette action et qu'elle ne doit être tentée **d'aucune autre manière** (autre commande, script, outil ou contournement). **Sans UI disponible** (mode non-interactif `--print`, RPC sans dialogues) → commande bloquée par défaut avec un message explicite.
